...
import { HiMusicalNote, HiPhoto, HiVideoCamera, HiUserPlus, HiChatBubbleLeft, HiArrowRightOnRectangle, HiPlay, HiUserMinus, HiCheck, HiClock } from 'react-icons/hi2';
...
const Profile = () => {
  ...
  const [friendStatus, setFriendStatus] = useState('none'); // none, friends, pending-sent, pending-received
  ...
  return (
    ...
    >
      ...
      <AnimatePresence mode="wait">
        ...
        {activeTab === 'photos' && (
          <motion.div key="photos" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="grid grid-cols-3 gap-1.5">
            {content.photos.map(photo => (
              <motion.div key={photo._id} whileHover={{ scale: 1.02 }} className="aspect-square rounded-xl overflow-hidden bg-white dark:bg-dark-card shadow-sm">
        ...
        {activeTab === 'videos' && (
          <motion.div key="videos" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="grid grid-cols-2 gap-3">
            {content.videos.map(video => (
              <div key={video._id} className="glass-card overflow-hidden">
                ...
                <p className="text-sm font-semibold truncate px-3 py-2.5">{video.title}</p>
              </div>
            ))}
            {content.videos.length === 0 && <p className="col-span-2 text-center py-12 text-gray-400 dark:text-gray-500 text-sm">No videos yet</p>}
          </motion.div>
        )}
      </AnimatePresence>
    </motion.div>
